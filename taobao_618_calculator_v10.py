...
def calc_gov_subsidy(products):
    """
    计算政府补贴
    核实结果：每件商品独立计算，按原价×补贴比例
    不是每种品类只补贴1件！
    """
    total_subsidy = 0
    eligible_products = []  # Returns (product_key, product_name, subsidy)
    
    for key, p in products.items() if isinstance(products, dict) else []:
        pass
    
    # products is a list of Product objects, we need to track their keys
    # We'll store (product_name, subsidy) and fix the printing
    for p in products:
        if p.gov_subsidy_eligible and p.gov_subsidy_rate > 0:
            subsidy = p.original_price * p.gov_subsidy_rate
            total_subsidy += subsidy
            eligible_products.append((p.name, p.gov_subsidy_rate, subsidy))
...
def solve():
    ...
    # 从购物车和截图提取的商品数据
    # price = 购物车折后价（已含店铺优惠）
    # original_price = 截图中的原价
    # gov_subsidy_rate = 根据截图计算（补贴金额/原价）
    all_products = {
        # 床垫类
        '蓝盒子Z1床垫': Product(
            ...
            gov_subsidy_rate=0.102,  # 487.66/4788 = 0.1018
        ),
        '林氏椰棕床垫': Product(
            '林氏椰棕床垫', 1686.37, '林氏家居', 'mattress',
            original_price=3238,
            gov_subsidy_eligible=True,
            gov_subsidy_rate=0.092,  # 297.59/3238 = 0.0919
        ),
        ...
        '林氏实木儿童床(含床垫)': Product(
            ...
            has_mattress=True,
            gov_subsidy_eligible=True,
            gov_subsidy_rate=0.0895,  # 356.95/3985 = 0.0896
        ),
        '林氏奶油风实木床(高配,含床垫)': Product(
            ...
            has_mattress=True,
            gov_subsidy_eligible=True,
            gov_subsidy_rate=0.0885,  # 399.72/4515 = 0.0885
        ),
        '林氏实木床1.8m标配(仅床架)': Product(
            ...
            has_mattress=False,
            gov_subsidy_eligible=True,
            gov_subsidy_rate=0.0901,  # 253.09/2809 = 0.0901
        ),
        ...
        '林氏奶油风沙发': Product(
            ...
            gov_subsidy_eligible=True,
            gov_subsidy_rate=0.0966,  # 346.48/3584.4 ≈ 0.0966
        ),
        '林氏原木风沙发': Product(
            '林氏原木风沙发', 3514.24, '林氏家居', 'sofa',
            original_price=6645,
            gov_subsidy_eligible=True,
            gov_subsidy_rate=0.0933,  # 620.16/6645 ≈ 0.0933
        ),
        ...
        '林氏岩板餐桌': Product(
            ...
            original_price=4429,
            gov_subsidy_eligible=True,
            gov_subsidy_rate=0.0921,  # 407.82/4429 ≈ 0.0921
        ),
        '林氏原木风餐桌': Product(
            '林氏原木风餐桌1桌4椅', 2099.52, '林氏家居', 'table',
            original_price=3406,
            gov_subsidy_eligible=True,
            gov_subsidy_rate=0.0894,  # 304.59/3406 ≈ 0.0894
        ),
    }

    print("=" * 75)
    print("淘宝618购物车最优购买方案计算器 v10（最终修正版）")
    print("=" * 75)
    print("\n✅ 已核实政策数据：")
    print("  • 跨店满减：每满300减50（全周期统一）")
    print("  • 88VIP消费券：满5000减650、满3000减400、满1500减180")
    print("  • 政府补贴：每件商品独立计算，按原价×比例（家具约9-10%）")
    ...
    print(f"原始总价（截图原价）：{sum(all_products[p].original_price for p in products_a):.2f} 元")
    print(f"\n最佳订单拆分：")
    for i, order in enumerate(best_a['orders']):
        ...
        if order['gov_subsidy'] > 0:
            ...
            for prod_name, rate, subsidy in order['eligible_gov_products']:
                print(f"      - {prod_name}: {subsidy:.2f} 元 (原价×{rate*100:.1f}%)")
    ...
    print(f"原始总价（截图原价）：{sum(all_products[p].original_price for p in products_b):.2f} 元")
    print(f"\n最佳订单拆分：")
    for i, order in enumerate(best_b['orders']):
        ...
        if order['gov_subsidy'] > 0:
            ...
            for prod_name, subsidy in order['eligible_gov_products']:
                print(f"      - {prod_name}: {subsidy:.2f} 元 (原价×{all_products[prod_name].gov_subsidy_rate*100:.1f}%)")
        print(f"    ★ 实付：{order['final']:.2f} 元")
    ...
    print("✅ 政策核实结果：")
    print("=" * 75)
    print("  1. 政府补贴：每件商品独立计算，按原价×比例（家具约9-10%）")
    print("     ❌ 不是'每种品类只补贴1件'，而是每件有标识的商品都可补贴")
